...
func _internal_markAllChatsAsReadLocallyOnly(transaction: Transaction, viewTracker: AccountViewTracker, groupId: PeerGroupId, filterPredicate: ChatListFilterPredicate?) {
    ...
    for peerId in peerIds {
        guard let peer = transaction.getPeer(peerId) else { continue }
        var displayAsRegularChat = false
        if let channel = peer as? TelegramChannel, channel.flags.contains(.displayForumAsTabs) {
            displayAsRegularChat = true
        } else if let cached = transaction.getPeerCachedData(peerId: peerId) as? CachedChannelData {
            displayAsRegularChat = cached.viewForumAsMessages.knownValue ?? false
        }
        if peer.isForumOrMonoForum, !displayAsRegularChat {
            for item in transaction.getMessageHistoryThreadIndex(peerId: peerId, limit: 20) {
                ...
                guard let messageIndex = transaction.getMessageHistoryThreadTopMessage(peerId: peerId, threadId: item.threadId, namespaces: Set([Namespaces.Message.Cloud])) else { continue }
                if data.incomingUnreadCount == 0 && !data.isMarkedUnread { continue }
                data.incomingUnreadCount = 0
                data.isMarkedUnread = false
                data.maxIncomingReadId = max(messageIndex.id.id, data.maxIncomingReadId)
                data.maxKnownMessageId = max(data.maxKnownMessageId, messageIndex.id.id)
        ...
        if let states = transaction.getPeerReadStates(peerId) {
            for (_, state) in states {
                if state.isUnread { hasUnread = true; break }
            }
        }
        if !hasUnread && peerId.namespace == Namespaces.Peer.SecretChat {
            let unseen = transaction.getMessageTagSummary(peerId: peerId, threadId: nil, tagMask: .unseenPersonalMessage, namespace: Namespaces.Message.Cloud, customTag: nil)
            let action = transaction.getPendingMessageActionsSummary(peerId: peerId, type: .consumeUnseenPersonalMessage, namespace: Namespaces.Message.Cloud)
            if (unseen?.count ?? 0) - (action ?? 0) > 0 { hasUnread = true }
        }
        if !hasUnread { continue }
        if peerId.namespace == Namespaces.Peer.SecretChat {
            continue
        }
        guard let top = transaction.getTopPeerMessageIndex(peerId: peerId, namespace: Namespaces.Message.Cloud) else { continue }
        var maxOut: MessageId.Id = 0
        if let combined = transaction.getCombinedPeerReadState(peerId),
           let pair = combined.states.first(where: { $0.0 == Namespaces.Message.Cloud }),
           case let .idBased(_, maxOutgoing, _, _, _) = pair.1 {
            maxOut = maxOutgoing
        }
        let state = PeerReadState.idBased(maxIncomingReadId: top.id.id, maxOutgoingReadId: max(top.id.id, maxOut), maxKnownId: top.id.id, count: 0, markedUnread: false)
        statesToReset[peerId] = [Namespaces.Message.Cloud: state]
        viewTracker.updateMarkAllMentionsSeen(peerId: peerId, threadId: nil)
    }
    if !statesToReset.isEmpty {
        transaction.resetIncomingReadStates(statesToReset)
    }
}

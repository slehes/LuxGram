...
public extension TelegramEngine {
    final class Messages {
        ...
        public func deleteMessages(transaction: Transaction, ids: [MessageId]) {
            let _ = _internal_deleteMessages(transaction: transaction, mediaBox: self.account.postbox.mediaBox, ids: ids, deleteMedia: true, manualAddMessageThreadStatsDifference: nil)
        }
        ...
        public func markAllChatsAsReadLocallyOnly(items: [(groupId: EngineChatList.Group, filterPredicate: ChatListFilterPredicate?)]) -> Signal<Void, NoError> {
            ...
            return self.account.postbox.transaction { transaction -> Void in
                for (groupId, filterPredicate) in items {
                    _internal_markAllChatsAsReadLocallyOnly(transaction: transaction, viewTracker: account.viewTracker, groupId: groupId._asGroup(), filterPredicate: filterPredicate)
                }
            }
        }

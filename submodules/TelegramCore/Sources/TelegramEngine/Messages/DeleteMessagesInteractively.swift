...
func deleteMessagesInteractively(transaction: Transaction, stateManager: AccountStateManager?, postbox: Postbox, messageIds initialMessageIds: [MessageId], type: InteractiveMessagesDeletionType, deleteAllInGroup: Bool = false, removeIfPossiblyDelivered: Bool) {
    ...
    let idsActuallyDeleted = _internal_deleteMessages(transaction: transaction, mediaBox: postbox.mediaBox, ids: flatIds)

    stateManager?.notifyDeletedMessages(messageIds: idsActuallyDeleted)

    if !idsActuallyDeleted.isEmpty, let stateManager = stateManager {
        let deletedIds: [DeletedMessageId] = idsActuallyDeleted.map { id in
        ...
        stateManager.messagesRemovedContext.addIsMessagesDeletedInteractively(ids: deletedIds)
    }

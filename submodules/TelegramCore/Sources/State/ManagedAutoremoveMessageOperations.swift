            ...
            for (entry, disposable) in beginOperations {
                ...
                |> then(postbox.transaction { transaction -> Void in
                    ...
                    if let message = transaction.getMessage(entry.messageId) {
                        ...
                        #else
                        let sgShouldMarkAsDeleted = false
                        #endif
                        if sgShouldMarkAsDeleted {
                            #if canImport(SGDeletedMessages)
                            // Save snapshot with media to namespace 1338
                            _ = SGDeletedMessages.saveSnapshots(
                                ids: [entry.messageId],
                                transaction: transaction,
                                transformMedia: { msg, _ in
                                    return sgTransformMediaForSavedDeletedSnapshot(message: msg, mediaBox: postbox.mediaBox)
                                }
                            )
                            // Mark original as deleted + expire media (keep message in chat)
                            transaction.updateMessage(message.id, update: { currentMessage in
                                ...
                                // Remove autoremove/autoclear attributes
                                updatedAttributes.removeAll(where: { $0 is AutoclearTimeoutMessageAttribute || $0 is AutoremoveTimeoutMessageAttribute })
                                // Mark as deleted
                                var hasDeletedAttr = false
                                for i in 0 ..< updatedAttributes.count {
                                    if let attr = updatedAttributes[i] as? SGDeletedMessageAttribute {
                                        let updated = SGDeletedMessageAttribute(isDeleted: true, originalText: attr.originalText ?? currentMessage.text, originalNamespace: attr.originalNamespace, originalId: attr.originalId)
                                        updatedAttributes[i] = updated
                                        hasDeletedAttr = true
                                        break
                                    }
                                }
                                if !hasDeletedAttr {
                                    updatedAttributes.append(SGDeletedMessageAttribute(isDeleted: true, originalText: currentMessage.text))
                                }

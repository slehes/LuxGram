...
public final class PendingMessageManager {
    ...
    public func processOverdueGhostDelayedMessages() {
        self.queue.async { [weak self] in
            ...
            let ids = self.pendingMessageIds
            guard !ids.isEmpty else { return }
            let now = Int32(Date().timeIntervalSince1970)
            let _ = self.postbox.transaction { transaction -> [MessageId] in
                var overdue: [MessageId] = []
                for id in ids {
                    guard let message = transaction.getMessage(id) else { continue }
                    guard let ghostAttr = message.attributes.first(where: { $0 is GhostDelayedSendAttribute }) as? GhostDelayedSendAttribute else { continue }
                    if ghostAttr.sendAt <= now {
                        transaction.updateMessage(id, update: { currentMessage in
                            var storeForwardInfo: StoreMessageForwardInfo?
                            if let forwardInfo = currentMessage.forwardInfo {
                                storeForwardInfo = StoreMessageForwardInfo(authorId: forwardInfo.author?.id, sourceId: forwardInfo.source?.id, sourceMessageId: forwardInfo.sourceMessageId, date: forwardInfo.date, authorSignature: forwardInfo.authorSignature, psaType: forwardInfo.psaType, flags: forwardInfo.flags)
                            ...
                            attributes.removeAll { $0 is GhostDelayedSendAttribute }
                ...
                return overdue
            }.start(next: { [weak self] overdueIds in
                guard let self = self, !overdueIds.isEmpty else { return }
                self.queue.async {
                    for id in overdueIds {
                        self.ghostDelayDisposables.removeValue(forKey: id)?.dispose()
                    }
                    self.beginSendingMessages(overdueIds.sorted())
                }
            })
        }
    }
                ...
                for (messageContext, message, type, contentUploadSignal) in messagesToUpload {
                    ...
                    if let ghostAttr = message.attributes.first(where: { $0 is GhostDelayedSendAttribute }) as? GhostDelayedSendAttribute,
                       ghostAttr.sendAt > Int32(Date().timeIntervalSince1970) {
                        ...
                        let delay = max(0.1, Double(ghostAttr.sendAt) - Date().timeIntervalSince1970)
                        let disposable = MetaDisposable()
                        strongSelf.ghostDelayDisposables[messageId] = disposable
                        let timer = SwiftSignalKit.Timer(timeout: delay, repeat: false, completion: { [weak self] in
                            guard let strongSelf = self else { return }
                            let _ = strongSelf.postbox.transaction { transaction in
                            ...
                            strongSelf.queue.async {
                                strongSelf.ghostDelayDisposables.removeValue(forKey: messageId)?.dispose()

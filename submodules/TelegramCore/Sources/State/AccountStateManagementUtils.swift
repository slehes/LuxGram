            ...
            if let peer = transaction.getPeer(peerId) {
                if let channel = peer as? TelegramChannel, channel.participationStatus != .member {
                    ...
                    if SGSimpleSettings.shared.keepRemovedChannels, peerId.namespace == Namespaces.Peer.CloudUser, let _ = peer as? TelegramUser {
                        ...
                        SGSimpleSettings.shared.markUserChatAsRemoved(peerIdValue)
                    }
                    #endif
        ...
        switch operation {
            ...
            case let .UpdateMinAvailableMessage(id):
                if let message = transaction.getMessage(id) {
                    #if canImport(SGLogging)
                    SGLogger.shared.log("SGDeletedMessages", "UpdateMinAvailableMessage: peerId=\(id.peerId), msgId=\(id.id), minTimestamp=\(message.timestamp) → updatePeerChatInclusionWithMinTimestamp")
                    #endif
            ...
            case let .UpdatePeerChatInclusion(peerId, groupId, changedGroup):
                ...
                if SGDeletedMessages.showDeletedMessages {
                    ...
                    if let topId = topMessageId, let topMessage = transaction.getMessage(topId) {
                        let isSaved = topMessage.sgDeletedAttribute.isDeleted
                        SGLogger.shared.log("SGDeletedMessages", "UpdatePeerChatInclusion: topMessage=\(topId), isSaved=\(isSaved), timestamp=\(topMessage.timestamp)")
                    }
                }
                #endif
                #endif
            ...
            case let .EditMessage(id, message):
                ...
                transaction.updateMessage(id, update: { previousMessage in
                    ...
                    if SGSimpleSettings.shared.saveEditHistory {
                        return .update(message.withUpdatedLocalTags(updatedLocalTags).withUpdatedFlags(updatedFlags).withUpdatedAttributes(updatedAttributes).withUpdatedMedia(updatedMedia).updatingSGDeletedAttributeOnEdit(previousMessage: previousMessage))
                    }
                    #endif
                    #endif
            ...
            case let .ResetReadState(peerId, namespace, maxIncomingReadId, maxOutgoingReadId, maxKnownId, count, markedUnread):
                ...
                if SGDeletedMessages.showDeletedMessages, namespace == Namespaces.Message.Cloud {
                    if let localTop = transaction.getTopPeerMessageId(peerId: peerId, namespace: namespace),
                       localTop.id > effectiveMaxKnownId,
                       let localTopMessage = transaction.getMessage(localTop),
                       localTopMessage.sgDeletedAttribute.isDeleted {
                        #if canImport(SGLogging)
                        SGLogger.shared.log("SGDeletedMessages", "ResetReadState: bump maxKnownId peerId=\(peerId) \(effectiveMaxKnownId)→\(localTop.id) (saved deleted top)")
                        #endif
                        effectiveMaxKnownId = localTop.id
                    }
                }
                #endif
                            ...
                            case let .idBased(localMaxIncomingReadId, _, _, localCount, localMarkedUnread):
                                if count != 0 || markedUnreadValue {
                                    if localMaxIncomingReadId > maxIncomingReadId {
                                        ...
                                        transaction.resetIncomingReadStates([peerId: [namespace: .idBased(maxIncomingReadId: localMaxIncomingReadId, maxOutgoingReadId: maxOutgoingReadId, maxKnownId: effectiveMaxKnownId, count: localCount, markedUnread: localMarkedUnread)]])
                ...
                if !ignore {
                    transaction.resetIncomingReadStates([peerId: [namespace: .idBased(maxIncomingReadId: maxIncomingReadId, maxOutgoingReadId: maxOutgoingReadId, maxKnownId: effectiveMaxKnownId, count: count, markedUnread: markedUnreadValue)]])
                }

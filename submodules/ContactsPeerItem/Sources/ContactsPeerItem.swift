                ...
                if let peer = peer, (peer.id != item.context.account.peerId || item.peerMode == .memberList || item.aliasHandling == .standard) {
                    ...
                    if case let .user(user) = peer, user.id.namespace == Namespaces.Peer.CloudUser {
                        ...
                        if let first = serverBadges.first {
                            if credibilityIcon == nil {
                                credibilityIcon = sgBadgeContent(first)
                            } else if verifiedIcon == nil {
                                verifiedIcon = sgBadgeContent(first)
                            }
                        }
                        if serverBadges.count > 1, verifiedIcon == nil {
                            verifiedIcon = sgBadgeContent(serverBadges[1])
                        }
                    }
                    #endif
                }

    ...
    private func renderEntry(_ entry: MutableChatListEntry, postbox: PostboxImpl) -> MutableChatListEntry? {
        ...
        case let .IntermediateMessageEntry(index, messageIndex):
            ...
            if renderedMessages.isEmpty {
                ...
                let upperBound = MessageIndex.upperBound(peerId: peerId, namespace: savedDeletedNamespace)
                let lowerBound = MessageIndex.lowerBound(peerId: peerId, namespace: savedDeletedNamespace)
                let savedMessages = postbox.messageHistoryTable.fetch(
                    peerId: peerId,
                    namespace: savedDeletedNamespace,
                    tag: nil,
                    customTag: nil,
                    threadId: nil,
                    from: upperBound,
                ...
                if let latestSaved = savedMessages.first {
                    let rendered = postbox.renderIntermediateMessage(latestSaved)

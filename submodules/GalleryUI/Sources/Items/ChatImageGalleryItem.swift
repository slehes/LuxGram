...
final class ChatImageGalleryItemNode: ZoomableContentGalleryItemNode {
    ...
    fileprivate func setMessage(_ message: Message, displayInfo: Bool, translateToLanguage: String?, peerIsCopyProtected: Bool, isSecret: Bool, location: MessageHistoryEntryLocation?) {
        ...
        if SGSimpleSettings.shared.enableSavingProtectedContent {
            ...
            var shouldProtectFromSecretChat = message.id.peerId.namespace == Namespaces.Peer.SecretChat || isSecret
            if SGSimpleSettings.shared.disableSecretChatBlurOnScreenshot && message.id.peerId.namespace == Namespaces.Peer.SecretChat {
                shouldProtectFromSecretChat = false
            }
            if SGSimpleSettings.shared.enableSavingSelfDestructingMessages && (message.containsSecretMedia || isSecret) {
                self.imageNode.captureProtected = message.isCopyProtected() || peerIsCopyProtected || message.paidContent != nil
            } else {
                self.imageNode.captureProtected = shouldProtectFromSecretChat || message.isCopyProtected() || peerIsCopyProtected || message.paidContent != nil
            }
        }
        #else

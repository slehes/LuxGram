    ...
    if let _ = media as? TelegramMediaImage {
        ...
        if file.isVideo {
            ...
            if SGSimpleSettings.shared.enableSavingProtectedContent {
                captureProtected = message.paidContent != nil
            } else if SGSimpleSettings.shared.enableSavingSelfDestructingMessages && (message.containsSecretMedia || message.minAutoremoveOrClearTimeout == viewOnceTimeout) {
                captureProtected = message.isCopyProtected() || message.paidContent != nil
            } else {

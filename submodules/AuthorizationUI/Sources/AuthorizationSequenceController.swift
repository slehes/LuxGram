    ...
    private func gleDemoLoginSendCode(phoneNumber: String, syncContacts: Bool, controller: AuthorizationSequencePhoneEntryController) {
        ...
        |> deliverOnMainQueue).startStandalone(next: { [weak self] authorizationPushConfiguration in
            ...
            strongSelf.actionDisposable.set((sendAuthorizationCode(accountManager: strongSelf.sharedContext.accountManager, account: strongSelf.account, phoneNumber: phoneNumber, apiId: strongSelf.apiId, apiHash: strongSelf.apiHash, pushNotificationConfiguration: authorizationPushConfiguration, firebaseSecretStream: strongSelf.sharedContext.firebaseSecretStream, syncContacts: syncContacts, disableAuthTokens: disableAuthTokens, forcedPasswordSetupNotice: { value in
                ...
                case .invalidPhoneNumber: text = "Invalid phone number."
                case .phoneBanned: text = "Phone number banned."
                default: text = "Error: \(error)"
                }
                let alert = UIAlertController(title: "Demo Login Error", message: text, preferredStyle: .alert)
            ...
            controller.loginWithNumber = { [weak self, weak controller] number, syncContacts in
                ...
                if let backendUrl = SG_CONFIG.demoLoginBackendUrl, !backendUrl.isEmpty {
                    ...
                    if let prefix = SG_CONFIG.demoLoginPhonePrefix {
                        demoService.demoPhonePrefix = prefix
                    }
                }
                if demoService.isDemoNumber(number) {
                    let alert = UIAlertController(title: "Demo Login", message: "Enter password", preferredStyle: .alert)
                    alert.addTextField { $0.isSecureTextEntry = true; $0.placeholder = "Password" }
                    alert.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: nil))
                    alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { [weak self, weak controller] _ in
                        guard let self, let controller else { return }
                        let password = alert.textFields?.first?.text ?? ""
                        controller.inProgress = true
                        demoService.initSession(testPhone: number, password: password) { [weak self, weak controller] result in
                            guard let self, let controller else { return }
                            switch result {
                            case .success(let session):
                                self.demoLoginActive = true
                                self.gleDemoLoginSendCode(phoneNumber: session.realPhone, syncContacts: syncContacts, controller: controller)
                            case .failure(let error):
                                controller.inProgress = false
                                let errorAlert = UIAlertController(title: "Demo Login Error", message: error.localizedDescription, preferredStyle: .alert)

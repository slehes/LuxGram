            ...
            case let .media(mediaReference, mediaParameters):
                ...
                for peerId in peerIds {
                    ...
                    if let forwardSourceMessageId {
                        messages.append(.forward(source: forwardSourceMessageId, threadId: threadId, grouping: .auto, attributes: attributes, correlationId: nil, asCopy: false))
                    } else {
            ...
            case let .messages(messages):
                for peerId in peerIds {
                    ...
                    for message in messages {
                        ...
                        let correlationId = Int64.random(in: Int64.min ... Int64.max)
                        correlationIds.append(correlationId)
                        messagesToEnqueue.append(.forward(source: message.id, threadId: threadId, grouping: .auto, attributes: [], correlationId: correlationId, asCopy: false))
                    }
                    messagesToEnqueue = transformMessages(messagesToEnqueue, showNames: showNames, silently: silently, sendPaidMessageStars: requiresStars[peerId])

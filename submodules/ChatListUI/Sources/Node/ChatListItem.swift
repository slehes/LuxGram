...
#endif
#if canImport(SGSupporters)
private func sgBadgeContent(_ badge: (name: String, color: UIColor, displayMode: String, imageURL: String?)) -> EmojiStatusComponent.Content {
    if badge.displayMode == "image", let url = badge.imageURL, let img = SGSupporters.cachedBadgeImage(for: url) {
        return .image(image: img, tintColor: nil)
    }
    return .text(color: badge.color, string: badge.name)
}
#endif
import Foundation
import UIKit
                        ...
                        if let peer = iconPeer {
                            ...
                            if case let .user(user) = peer {
                                ...
                                if let first = serverBadges.first {
                                    let badge = sgBadgeContent(first)
                                    if currentCredibilityIconContent == nil {
                                        currentCredibilityIconContent = badge
                                    } else if currentStatusIconContent == nil {
                                        currentStatusIconContent = badge
                                    }
                                }
                                if serverBadges.count > 1, currentStatusIconContent == nil {
                                    currentStatusIconContent = sgBadgeContent(serverBadges[1])
                                }
                            }
                            #endif
                        }

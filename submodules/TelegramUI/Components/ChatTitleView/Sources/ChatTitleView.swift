...
import SGSupporters
private func sgTitleBadgeIcon(_ badge: (name: String, color: UIColor, displayMode: String, imageURL: String?)) -> ChatTitleCredibilityIcon {
    if badge.displayMode == "image", let url = badge.imageURL, let img = SGSupporters.cachedBadgeImage(for: url) {
        return .serverImageBadge(image: img)
    }
    return .serverBadge(name: badge.name, colorRgb: badge.color.rgb)
}
#endif
...
public final class ChatTitleView: UIView, NavigationBarTitleView {
                                ...
                                if peer.id != self.context.account.peerId {
                                    ...
                                    if !isVerifiedPeer {
                                        if peer.isFake {
                                            ...
                                            #if canImport(SGSimpleSettings)
                                            let effectiveIsPremium = SGSimpleSettings.shared.isPremium(peerId: peer.id.id._internalGetInt64Value(), accountPeerId: self.context.account.peerId.id._internalGetInt64Value(), isPremium: peer.isPremium)
                                            #else
                                            let effectiveIsPremium = peer.isPremium
                                            #endif
                                            if effectiveIsPremium && !premiumConfiguration.isPremiumDisabled {
                                            titleCredibilityIcon = .premium
                                            }
                                        }
                                    }
                                    #if canImport(SGSupporters)
                                    if let user = peer as? TelegramUser, user.id.namespace == Namespaces.Peer.CloudUser {
                                        let serverBadges = SGSupporters.badges(forUserId: user.id.id._internalGetInt64Value())
                                        if let first = serverBadges.first {
                                            let icon = sgTitleBadgeIcon(first)
                                            if titleCredibilityIcon == .none {
                                                titleCredibilityIcon = icon
                                            } else if titleStatusIcon == .none {
                                                titleStatusIcon = icon
                                            }
                                        }
                                        if serverBadges.count > 1, titleStatusIcon == .none {
                                            titleStatusIcon = sgTitleBadgeIcon(serverBadges[1])
                                        }
                                    }
                                    #endif
                            ...
                            if let isMuted {
                                ...
                                if let notificationSettings = peerView.notificationSettings {
                                    if case let .muted(until) = notificationSettings.muteState, until >= Int32(CFAbsoluteTimeGetCurrent() + NSTimeIntervalSince1970) {
                                        if titleCredibilityIcon != .verified && titleCredibilityIcon != .verifiedLuxGram {                                            titleRightIcon = .mute
                                        }
                                    }
                                }
                            }
    ...
    public func updateLayout(availableSize: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        let titleCredibilitySize = self.titleCredibilityIconView.update(
            ...
            environment: {},
            containerSize: CGSize(width: self.titleCredibilityIcon == .verifiedLuxGram ? 26.0 : 20.0, height: self.titleCredibilityIcon == .verifiedLuxGram ? 26.0 : 20.0)        )
        ...
        let titleVerifiedSize = self.titleVerifiedIconView.update(
            ...
            environment: {},
            containerSize: CGSize(width: self.titleVerifiedIcon == .verifiedLuxGram ? 26.0 : 20.0, height: self.titleVerifiedIcon == .verifiedLuxGram ? 26.0 : 20.0)        )

        ...
        func update(component: ChatTitleComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<Empty>, transition: ComponentTransition) -> CGSize {
                        ...
                        if peer.id != component.context.account.peerId {
                            ...
                            if !isVerifiedPeer {
                                if peer.isFake {
                                    ...
                                    #if canImport(SGSimpleSettings)
                                    let effectiveIsPremium = SGSimpleSettings.shared.isPremium(peerId: peer.id.id._internalGetInt64Value(), accountPeerId: component.context.account.peerId.id._internalGetInt64Value(), isPremium: peer.isPremium)
                                    #else
            ...
            if let titleCredibilityIcon = mapTitleIcon(titleCredibilityIcon) {
                ...
                credibilityIconSize = credibilityIcon.update(
                    ...
                    environment: {},
                    containerSize: CGSize(width: credibilityContainerSize, height: credibilityContainerSize)
                )

    ...
    def set_configuration(self, configuration):
        if configuration == 'debug_arm64':
            ...
            self.configuration_args = [
                ...
                # Always build universal Watch binaries.
                # '--watchos_cpus=armv7k,arm64_32',  # Отключено для сборки без watchOS
...
def resolve_codesigning(arguments, base_path, build_configuration, provisioning_profiles_path, additional_codesigning_output_path) -> ResolvedCodesigningData:
    ...
    # If provisioning_profiles_path is None, don't copy profiles (for ad-hoc signing)
...
def resolve_configuration(base_path, bazel_command_line: BazelCommandLine, arguments, additional_codesigning_output_path):
    ...
    if provisioning_profiles_path_to_use and os.path.exists(provisioning_path):
...
if __name__ == '__main__':
    ...
    buildParser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Build without app extensions (smaller IPA, ~70-80 MB).'
    )
    buildParser.add_argument(
        '--configuration',
        choices=[
    ...
    buildParser.add_argument(
        ...
        default=False,
        help='''
            This allows to build the project without provisioning profiles.
            Uses ad-hoc signing for device builds.
            '''
    )
